...
val data = vecAssembler.getInputCols.zip(dtModel.featureImportances.toArray)
val columns = Array("feature", "Importance")
val featuresDF = spark.createDataFrame(data).toDF(columns: _*)
...
val sortedData = featuresDF.orderBy($"importance".desc).select("feature").collect.map(_.toSeq).flatten
