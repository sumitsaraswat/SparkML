...
# MAGIC | 10m  | **Break**                                               ||
# MAGIC | 40m  | **[Hyperparameter Tuning Lab]($./Labs/ML 07L - Hyperparameter Tuning Lab)**  | Perform grid search on a random forest <br/>Get the feature importances across the forest <br/>Save the model <br/>Identify differences between Sklearn's Random Forest and SparkML's |
...
# MAGIC | 15m    | **[MLlib Deployment Options]($./ML 09 - MLlib Deployment Options)  (optional)**    | Discuss batch, streaming, and real-time use cases and how MLlib/Spark fit into each of those |
# MAGIC | 20m    | **[XGBoost]($./ML 10 - XGBoost)  (optional)**    | Using 3rd party libraries with Spark <br/>Discuss gradient boosted trees and their variants" |
# MAGIC | 10m  | **Break**                                               ||
# MAGIC | 15m    | **[Inference with Pandas UDFs]($./ML 11 - Inference with Pandas UDFs)**    | Build a single-node ML model, but apply in parallel using Pandas Scalar Iterator UDF & mapInPandas (introduced in Spark 3.0) |
# MAGIC | 15m    | **[Training with Pandas UDFs]($./ML 12 - Training with Pandas UDFs)**    | Build a single-node ML model for each IoT Device using applyInPandas (introduced in Spark 3.0) and track it with MLflow |
# MAGIC | 20m    | **[Pandas UDFs Lab]($./Labs/ML 12L - Pandas UDF Lab)**    | Lab to apply a single-node ML model at scale |
# MAGIC | 15m  | **[Koalas]($./ML 13 - Koalas)** | Use the new open-source library to write Pandas code that distributes using Spark under the hood|  
# MAGIC | 10m  | **Break**                                               ||
# MAGIC | 100m    | **[Capstone Project]($./ML 14 - Capstone Project) (optional)**    | Work in a small team to apply the skills you learned throughout the course to a new dataset |
# MAGIC 
# MAGIC **Optional**: Notebooks in the electives folder
