...
val filePath = "dbfs:/mnt/training/airbnb/sf-listings/sf-listings-2019-03-06.csv"
...
fixedPriceDF.filter($"price" === 0).count
...
val posPricesDF = fixedPriceDF.filter($"price" > 0)
...
val minNightsDF = posPricesDF.filter($"minimum_nights" <= 365)
...
val integerColumns = for (x <- minNightsDF.schema.fields if (x.dataType == IntegerType)) yield x.name  
var doublesDF = minNightsDF
...
val imputerModel = imputer.fit(doublesDF)
val imputedDF = imputerModel.transform(doublesDF)
...
val outputPath = userhome + "/machine-learning-s/airbnb-cleansed.delta"
